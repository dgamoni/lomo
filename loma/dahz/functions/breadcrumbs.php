<?php
/*-----------------------------------------------------------------------------------*/
/* dahz_breadcrumbs() - Custom breadcrumb generator function  */
/*
/* Params:
/*
/* Arguments Array:
 ...
 *
 * dahz_breadcrumbs() shows a breadcrumb for all types of pages.  Themes and plugins can filter $args or input directly.
...
function dahz_breadcrumbs( $args = array() ) {
	...
	elseif ( is_home() ) {
		...
		$trail = array_merge( $trail, dahz_breadcrumbs_get_parents( $home_page->post_parent, '' ) );
	...
	elseif ( is_singular() ) {
		...
		if ( 'attachment' == $post_type ) {
			...
			if ( !empty( $path ) && '/' != $path )
				$trail = array_merge( $trail, dahz_breadcrumbs_get_parents( '', $path ) );
		}
		...
		if ( empty( $path ) && 0 !== $parent || 'attachment' == $post_type )
			$trail = array_merge( $trail, dahz_breadcrumbs_get_parents( $parent, '' ) );
		...
		if ( 'post' == $post_type && $show_posts_page == true && 'page' == get_option( 'show_on_front' ) ) {
			...
			if ( $posts_page != '' && is_numeric( $posts_page ) ) {
				$trail = array_merge( $trail, dahz_breadcrumbs_get_parents( $posts_page, '' ) );
			}
		}
	...
	elseif ( is_archive() ) {
		...
		if ( is_tax() || is_category() || is_tag() ) {
			...
			if ( is_taxonomy_hierarchical( $term->taxonomy ) && $term->parent )
				$trail = array_merge( $trail, dahz_breadcrumbs_get_term_parents( $term->parent, $term->taxonomy ) );
		...
		elseif ( is_date() ) {
			...
			if ( $wp_rewrite->front )
				$trail = array_merge( $trail, dahz_breadcrumbs_get_parents( '', $wp_rewrite->front ) );
	...
	if ( $echo )
		...
		return $breadcrumb;
} // End dahz_breadcrumbs()

if ( ! function_exists( 'df_set_default_breadcrumb_taxonomies' ) ) {
/**
 * Cater for dahztheme post types where we know the taxonomy. These should be done in each plugin, in future.
 * @since  6.0.0
...
function df_set_default_breadcrumb_taxonomies ( $args ) {
	...
	return $args;
} // End df_set_default_breadcrumb_taxonomies()
}
add_filter( 'dahz_breadcrumbs_args', 'df_set_default_breadcrumb_taxonomies' );

if ( ! function_exists( 'df_maybe_add_shop_page_link' ) ) {
/**
 ...
 */
function df_maybe_add_shop_page_link ( $trail ) {
...
add_filter( 'dahz_breadcrumbs_trail', 'df_maybe_add_shop_page_link' );

/*-----------------------------------------------------------------------------------*/
/* dahz_breadcrumbs_get_parents() - Retrieve the parents of the current page/post */
/*-----------------------------------------------------------------------------------*/
/**
...
function dahz_breadcrumbs_get_parents( $post_id = '', $path = '' ) {
	...
	return $trail;
} // End dahz_breadcrumbs_get_parents()

/*-----------------------------------------------------------------------------------*/
/* dahz_breadcrumbs_get_term_parents() - Retrieve the parents of the current term */
/*-----------------------------------------------------------------------------------*/
/**
...
function dahz_breadcrumbs_get_term_parents( $parent_id = '', $taxonomy = '' ) {
	...
	return $trail;
} // End dahz_breadcrumbs_get_term_parents()
...
if ( ! function_exists( 'dahz_get_term_parents' ) ) {
