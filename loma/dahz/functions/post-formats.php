...
add_action( 'wp_loaded', 'dahz_structured_post_formats', 1 );
 ...
 */
function dahz_aside_infinity( $content ) {
	...
	if ( has_post_format( 'aside' ) && !is_singular() && !post_password_required() ) {
		...
		$content .= ' ' . apply_filters( 'dahz_aside_infinity', $infinity );
	}
 ...
 * @return string
 */
function dahz_image_content( $content ) {
 ...
 * @return string $content
 */
function dahz_quote_content( $content ) {
 ...
 * @return array
 */
function dahz_get_the_post_format_chat( $content ) {

	/* Allow the separator (separator for speaker/text) to be filtered. */
	$separator = apply_filters( 'dahz_post_format_chat_separator', ':' );
 ...
 * @since  1.2.1
 * @access public
 * @global array   $_dahz_post_chat_ids  An array of IDs for the chat rows based on the author.
 * @param  string  $content                The content of the post.
 * @return string  $chat_output            The formatted content of the post.
 */
function dahz_chat_content( $content ) {
	...
	$stanzas = dahz_get_the_post_format_chat( $content );
	...
	foreach ( $stanzas as $stanza ) {
		...
		foreach ( $stanza as $row ) {
			...
			$speaker_id = dahz_chat_row_id( $chat_author );
			...
			if ( !empty( $chat_author ) )
				$chat_output .= "\n\t\t\t\t\t" . '<div class="chat-author ' . sanitize_html_class( strtolower( "chat-author-{$chat_author}" ) ) . ' vcard">' . $time . '<cite class="fn">' . apply_filters( 'dahz_post_format_chat_author', $chat_author, $speaker_id ) . '</cite>:</div>';

			/* Add the chat row text. */
			$chat_output .= "\n\t\t\t\t\t" . '<div class="chat-text">' . str_replace( array( "\r", "\n", "\t" ), '', apply_filters( 'dahz_post_format_chat_text', $chat_text, $chat_author, $speaker_id ) ) . '</div>';
 ...
 */
function dahz_chat_row_id( $chat_author ) {
	global $_dahz_post_chat_ids;

	/* Let's sanitize the chat author to avoid craziness and differences like "John" and "john". */
	$chat_author = strtolower( strip_tags( $chat_author ) );

	/* Add the chat author to the array. */
	$_dahz_post_chat_ids[] = $chat_author;

	/* Make sure the array only holds unique values. */
	$_dahz_post_chat_ids = array_unique( $_dahz_post_chat_ids );

	/* Return the array key for the chat author and add "1" to avoid an ID of "0". */
	return absint( array_search( $chat_author, $_dahz_post_chat_ids ) ) + 1;
}
