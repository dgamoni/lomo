  ...
  function like_post($post_id, $action = 'get'){
    ...
    switch($action) {
      ...
      case 'get':
        ...
        return '<i class="fa fa-heart-o"></i><span class="df-like-count">'. $like_count .'</span>';
        break;
...
function df_like_latest_posts() {
  global $df_like;
  return $df_like->add_df_like(); 
}

function df_like_portfolio_list() {
  global $df_like;
  return $df_like->add_df_like_portfolio_list(); 
}

if ( ! function_exists('df_like_localization') ) {
function df_like_localization() {
      wp_localize_script( 'df-main', 'dfLike', array(
      'ajaxurl' => admin_url('admin-ajax.php')
    ));
 }
 add_action( 'df_add_localize_script',  'df_like_localization');
}