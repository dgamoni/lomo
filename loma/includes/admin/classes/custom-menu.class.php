...
class Df_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $current_id = 0 ) {
			...
			$header_fixed = df_options('header_navbar_position');
			...
			if ( !empty($args->please_be_mega_menu) ) {
				...
				if ( $this->df_fat_menu && !empty($args->please_be_fat) ) {
					...
					if ( 0 == $depth ) {
					   ...
					   if ( !empty($item->df_mega_menu_width_header_fixed) && ( $header_fixed == 'fixed-left' || $header_fixed == 'fixed-right' )  ) {
							$classes[] = 'mega-width-header-fixed'.' menu-data-width-'.$item->df_mega_menu_width_header_fixed.'-size';
						}
